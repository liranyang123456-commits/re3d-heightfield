...
class ImageProcessor:
    ...
    class _PoseOptimizationModel(nn.Module):
        ...
        def forward(self):
            ...
            # 计算轮廓损失（使用alpha通道，参考示例代码）
            alpha_rendered = image[0, ..., 3]
            alpha_rendered = torch.clamp(alpha_rendered, 0, 1)
            
            # 使用MSE损失（参考示例代码）
            loss = torch.sum((alpha_rendered - self.image_ref) ** 2)
    ...
    def _optimize_pose_with_rendering(self, prev_frame_idx, current_frame_idx, 
        ...
        try:
            ...
            _bs = os.environ.get("RASTER_BIN_SIZE", "auto")
            raster_settings_silhouette = RasterizationSettings(
                image_size=(image_height, image_width),
                blur_radius=np.log(1. / 1e-4 - 1.) * blend_params.sigma,
                faces_per_pixel=100,
                bin_size=(None if _bs == "auto" else int(_bs)),
                max_faces_per_bin=int(os.environ.get("RASTER_MAX_FACES_PER_BIN", "500000")),
            )
            ...
            for iteration in loop:
                ...
                # 早停条件（参考示例代码）
                if loss_value < 200:  # 示例代码中的早停条件
                    if hasattr(loop, 'set_description'):
                        loop.set_description(f'Converged (loss {loss_value:.4f})')
                    break
                
                # 打印进度
                if iteration % 10 == 0:
                    print(f"    [迭代 {iteration:3d}/{max_iterations}] 损失: {loss_value:.6f}")
...
def main():
    ...
    # 数据路径：图片文件夹
    input_image_folder = r"D:\reloc3r\Data_IMU_Camera_Pose_5\Lines_Photo _2"
